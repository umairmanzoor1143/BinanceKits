...
export function generateArray(number: number, data?: any) {
  ...
  return Array.from({ length }, (_, index) => (data ? { ...data, id: index + 1 } : index));
}
...
export const getUser = () => {
  return getLocalStorage('auth_user') as IUser;
};
...
export function getCSSPropertyValue(propertyValue: string) {
  ...
  return getComputedStyle(document.querySelector('body') as HTMLElement)?.getPropertyValue(
    propertyValue,
  );
}
